...
class _AddAdminViewState extends State<AddAdminView> {
  ...
  Future<void> _createAdmin(String username, String fullName, String email, String password) async {
    // Generate a fake teacher with valid data
    final adminData = AdminCreate.fromJson({
      "fullName": fullName, 
      "email": email, 
      "username": username, 
      "password": password, 
      ...
      print("Admin Data: ${adminData.toJson()}");

    try {
      // Send the fake teacher to the API
        await _adminService.createAdmin(adminData, token);
      //after success
      context.go('/dashboard/admins/all-admins');
            ...
            child: ElevatedButton(
              onPressed: () {
                ...
                if (_browserDefaultFormKey.currentState!.validate()) {
                  _createAdmin(_usernameController.text, _fullNameController.text, _emailController.text, _passwordController.text);
                }
              },
